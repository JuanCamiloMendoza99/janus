...
**11. Records that vanished.** *"About 400 records that were there yesterday are
gone this morning. Not in the archived filter, not in trash. We ran the sync at
02:00 as usual."* — `technical`, `critical`, `negative`, `escalate`. This is the
severity trap. Nothing is *exposed*, one customer is affected, and the tone is
calm, so the pull is toward `high` — but data was **lost**, and lost or corrupted
data is `critical` on its own, exactly like exposed data. The count of records
and the number of customers do not enter into it. Escalates because data was lost
at any severity.

**12. The relationship is the problem.** *"I have been a customer three years and
the service has gone downhill. Every time I write in I get a canned response.
Nobody takes ownership. I am considering whether to renew."* — `complaint`,
`medium`, `negative`, `route_to_human`. No defect an engineer could open, so
`complaint` not `technical`. But `medium`, not `low`: a multi-year account
stating it may not renew is real business damage accruing over time, which is
what `medium` severity is for. `low` is for a typo or a how-to; this needs a
human who can own the account, promptly.

**13. A refund the policy does not owe.** *"I downgraded from Scale to Growth on
the 3rd and expected a partial refund for the rest of the month. Nothing came
through. Can you process it?"* — `billing`, `low`, `neutral`, `route_to_human`.
Plan changes are `billing`. The reflex is `high` because the word "refund" reads
as money — but no money moved *incorrectly*: the documented behaviour is that
downgrades are not pro-rated, so the system did the right thing and the customer
is asking about policy. A question with no defect and no mis-charge is `low`. The
money-raises-the-floor rule fires only when funds moved *wrongly*.

**14. Billing topic, broken machine.** *"Our card expired. I went to Billing to
update it and the save button does nothing — no error, the page just sits
there."* — `technical`, `high`, `neutral`, `route_to_human`. Every word is about
billing, so the queue looks obvious, but ask who is wrong: the amount is fine and
the *software* is broken. That makes it `technical`. `high` because the customer
is blocked from fixing a payment method and heads toward a suspension they cannot
prevent — a core workflow broken for one customer with no working path.

**15. A deadline with legal weight.** *"We have a GDPR deletion request from a
former employee, legally required within 30 days, and we cannot find any way to
delete their data on your platform. Deadline is in 11 days."* — `technical`,
`high`, `negative`, `escalate`. The missing capability is a product gap, so
`technical`, not `account_access` and not `feature_request` — the customer cannot
meet a stated **legal obligation** because the product does not do what
compliance requires. Severity `high` on impact, but the escalation rule fires on
the regulatory consequence regardless of severity: a compliance deadline is one
of the named escalation triggers.

**16. Cold outreach.** *"Hi! I noticed you could be getting more traffic. Our
agency does SEO for SaaS companies — want a free audit?"* — `other`, `low`,
`neutral`, `close`. Not a support ticket at all, so `other`, and `close` is the
correct disposition rather than `route_to_human`: nobody needs to reply. `close`
is a real decision here, used because there is genuinely nothing to do — not to
dispose of a ticket that was merely hard to classify.

**17. A plain question, no tools this turn.** *"Where in settings do I change my
account timezone? I looked under Profile and could not find it."* — `other`,
`low`, `neutral`, `route_to_human`. A `low`-severity single question is exactly
what `auto_reply` is for — *but only when `search_kb` returned a matching
article*, and no tools are available in this request. So the rule holds: never
`auto_reply` without a confirmed article, and route to a human instead. The
verdict changes with the tools available, not with the ticket.
